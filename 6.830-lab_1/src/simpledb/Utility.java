...
public class Utility {
    ...
    public static TupleDesc getTupleDesc(Type[] t, int n) 
    {
    ...
    public static Tuple getTuple(Field[] tupledata, int width) {
    	...
    	for (int i = 0; i < width; ++i)
    	{
    		types[i] = tupledata[i].getType();
    	}
    	Tuple tup = new Tuple(getTupleDesc(types, width));
    	for (int i = 0; i < width; ++i)
    	{
    		if (tupledata[i] instanceof IntField)
    		{
    			IntField f = (IntField)tupledata[i];
    			tup.setField(i, new IntField(f.getValue()));
    		}
    		else
    		{
    			StringField f = (StringField) tupledata[i];
    			tup.setField(i, new StringField(f.getValue(), f.getValue().length()));
    		}
    	}
    	return tup;
    }
