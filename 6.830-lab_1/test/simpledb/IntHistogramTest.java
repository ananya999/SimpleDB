...
public class IntHistogramTest {
	...
	@Test public void orderOfGrowthTest() {
		...
		for (int c = 0; c < 33554432; c++) {
			h.addValue(new IntField((c * 23) % 101));	// Pseudo-random number; at least get a distribution
		}
		...
		for (int c = 0; c < 101; c++) {
			selectivity += h.estimateSelectivity(Op.EQUALS, new IntField(c));
		}
	...
	@Test public void negativeRangeTest() {
		...
		for (int c = -60; c <= -10; c++) {
			...
			h.estimateSelectivity(Op.EQUALS, new IntField(c));
		}
	...
	@Test public void opEqualsTest() {
		...
		Assert.assertTrue(h.estimateSelectivity(Op.EQUALS, new IntField(-1)) == 0);
	}
	...
	@Test public void opGreaterThanOrEqualsTest() {
		...
		Assert.assertTrue(h.estimateSelectivity(Op.GREATER_THAN_OR_EQ, new IntField(3)) > 0.45);
